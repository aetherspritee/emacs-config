...
- [[#default-compilation-for-different-languages][default compilation for different languages]]
...
# Defining some functions for a git diff module ill use instead of the default one.
# #+begin_src emacs-lisp
...
* default compilation for different languages
...
https://www.youtube.com/watch?v=6oeE52bIFyE
Set a per line compile command by adding // =-*- compile-command: "gcc -Wall -O3 -o f file.c" -*-= at the top of the file.
#+begin_src emacs-lisp
    (defun my/compile-python ()
      "bruh"
    (setq-local compile-command
